...
As
	...
	****************************************************************/
	...
	Begin -- <a1>
		...
		WHERE FileName = @organismDBFileName
		
		SELECT @myError = @@error, @myRowCount = @@rowcount
		
		If @myError <> 0
		Begin
			Set @msg = 'Database retrieval error during Organsim DB Filename check (Protein_Sequences.V_Legacy_Static_File_Locations)'
			Set @myError = -50010
			Set @message = @msg
			RAISERROR (@msg, 10, 1)
			return @myError
	...
	****************************************************************/
	...
	Begin -- <a2>
		...
		Begin -- <b2>
			...
			SELECT @tmpCollName = Collection_Name
			FROM @collListTable
			WHERE Collection_ID = @itemCounter
			
			SELECT @myError = @@error, @myRowCount = @@rowcount
			
			Set @tmpExtPosition = CHARINDEX('.fasta', @tmpCollName)
			If @tmpExtPosition > 0
			Begin
			...
			If @myRowCount = 0
			Begin
				Set @msg = '"' + @tmpCollName + '" was not found in the Protein Collection List'
				Set @message = @msg
				RAISERROR (@msg, 10, 1)
				return -50001
			End
			...
			Begin -- <c2>
				...
				WHERE Login_Name LIKE '%' + @ownerPRN + '%' AND
						Protein_Collection_ID = @tmpCollectionID
				
				SELECT @myError = @@error, @myRowCount = @@rowcount		
						
				If @myRowCount = 0
				Begin
					Set @msg = @ownerPRN + ' is not authorized for the encrypted collection "' + @tmpCollName + '"'
					Set @message = @msg
					RAISERROR (@msg, 10, 1)
					return -50020
				End
