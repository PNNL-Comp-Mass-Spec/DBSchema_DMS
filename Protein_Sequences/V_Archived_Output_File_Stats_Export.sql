...
AS
SELECT AOF.Archived_File_ID, AOF.Filesize, 
    COUNT(PC.Protein_Collection_ID) 
    AS Protein_Collection_Count, SUM(PC.NumProteins) 
    AS Protein_Count, SUM(PC.NumResidues) AS Residue_Count, 
    dbo.udfGetFileNameFromPath(AOF.Archived_File_Path) 
    AS Archived_File_Name
...
GROUP BY AOF.Archived_File_ID, AOF.Filesize, 
    dbo.udfGetFileNameFromPath(AOF.Archived_File_Path)
