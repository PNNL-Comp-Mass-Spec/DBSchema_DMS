...
SELECT  AJ.AJ_jobID AS Job ,
        ...
        CASE WHEN AJ.AJ_Purged = 0
             THEN ISNULL(SPath.SP_vol_name_client + SPath.SP_path
                         + ISNULL(DS.DS_folder_name, DS.Dataset_Num) + '\'
                         + AJ.AJ_resultsFolderName, '')
             ELSE ISNULL(DAP.Archive_Path + '\' + ISNULL(DS.DS_folder_name,
                                                         DS.Dataset_Num) + '\'
                         + AJ.AJ_resultsFolderName, '')
        END AS Folder ,
        ISNULL(DAP.Archive_Path + '\' + ISNULL(DS.DS_folder_name, DS.Dataset_Num) + '\' + AJ.AJ_resultsFolderName, '') AS Archive_Path,
