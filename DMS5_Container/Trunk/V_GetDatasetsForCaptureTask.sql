/****** Object:  View [dbo].[V_GetDatasetsForCaptureTask] ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE VIEW dbo.V_GetDatasetsForCaptureTask
AS
SELECT TOP 100 PERCENT dbo.T_Dataset.Dataset_Num AS Dataset,
    ...
    dbo.T_Dataset.Dataset_ID, dbo.T_Dataset.DS_rating, 
    dbo.T_Instrument_Name.IN_class AS InstrumentClass, 
    dbo.DatasetPreference(dbo.T_Dataset.Dataset_Num) 
    AS Preference, dbo.T_Instrument_Name.Instrument_ID
FROM dbo.T_Dataset INNER JOIN
    dbo.v_assigned_storage ON 
    dbo.T_Dataset.DS_instrument_name_ID = dbo.v_assigned_storage.Instrument_ID
     INNER JOIN
    dbo.T_Instrument_Name ON 
    dbo.T_Dataset.DS_instrument_name_ID = dbo.T_Instrument_Name.Instrument_ID
WHERE (dbo.T_Dataset.DS_state_ID = 1)
ORDER BY dbo.DatasetPreference(dbo.T_Dataset.Dataset_Num) DESC,
     dbo.T_Dataset.Dataset_ID

GO
GRANT VIEW DEFINITION ON [dbo].[V_GetDatasetsForCaptureTask] TO [PNL\D3M578] AS [dbo]
GO
GRANT VIEW DEFINITION ON [dbo].[V_GetDatasetsForCaptureTask] TO [PNL\D3M580] AS [dbo]
GO
