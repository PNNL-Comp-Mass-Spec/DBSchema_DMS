/****** Object:  View [dbo].[BlitzFirst_FileStats_Deltas] ******/
SET ANSI_NULLS ON
GO
...
INNER JOIN [dbo].[BlitzFirst_FileStats] fPrior ON f.ServerName = fPrior.ServerName AND f.DatabaseID = fPrior.DatabaseID AND f.FileID = fPrior.FileID AND f.CheckDate > fPrior.CheckDate
LEFT OUTER JOIN [dbo].[BlitzFirst_FileStats] fMiddle ON f.ServerName = fMiddle.ServerName AND f.DatabaseID = fMiddle.DatabaseID AND f.FileID = fMiddle.FileID AND f.CheckDate > fMiddle.CheckDate AND fMiddle.CheckDate > fPrior.CheckDate
