/****** Object:  View [dbo].[BlitzFirst_WaitStats_Deltas] ******/
SET ANSI_NULLS ON
GO
...
INNER JOIN [dbo].[BlitzFirst_WaitStats] wPrior ON w.ServerName = wPrior.ServerName AND w.wait_type = wPrior.wait_type AND w.CheckDate > wPrior.CheckDate
LEFT OUTER JOIN [dbo].[BlitzFirst_WaitStats] wMiddle ON w.ServerName = wMiddle.ServerName AND w.wait_type = wMiddle.wait_type AND w.CheckDate > wMiddle.CheckDate AND wMiddle.CheckDate > wPrior.CheckDate
