...
GO
CREATE VIEW V_Processor_Status_Warnings
AS
...
WHERE  (PS.Monitor_Processor <> 0 AND 
        ...
        (PS.Mgr_Status Like '%Error' OR PS.Mgr_Status LIKE 'Disabled%')
	   ) OR
       (PS.Monitor_Processor <> 0 AND NOT (StaleQ.Status_State IS NULL)) OR
       (PS.Monitor_Processor <> 0 AND NOT (NodeCountErrorQ.Status_State IS NULL))
