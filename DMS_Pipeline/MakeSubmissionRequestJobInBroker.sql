/****** Object:  StoredProcedure [dbo].[MakeSubmissionRequestJobInBroker] ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[MakeSubmissionRequestJobInBroker]
/****************************************************
**
**	Desc: 
...
**
**	Auth:	grk
**			05/18/2010 grk - Initial release
**			05/25/2010 grk - dataset name "Aggregation"
**			06/03/2010 grk - fix dataset job coverage verification
**			09/17/2010 jds - Added @targetFileSpecs as a parameter
*****************************************************/
(
	@DataPackageID INT,
	@PRIDEMzXMLInputFile VARCHAR(128),
	@Comment varchar(512),
	@Job int OUTPUT,
...
AS
	...
	BEGIN TRY
		...
		SET @hits = 0
		--
		SELECT @hits = @hits + COUNT(*) FROM dbo.CheckDataPackageJobs(@DataPackageID, 'MSXML_Gen', 'DatasetsWithoutJobs')
		...
		INSERT INTO @paramTab ([Section], [Name], [Value]) VALUES ('JobParameters', 'PRIDEMzXMLInputFile', @PRIDEMzXMLInputFile)
...
GO
GRANT EXECUTE ON [dbo].[MakeSubmissionRequestJobInBroker] TO [DMS_SP_User] AS [dbo]
GO
GRANT VIEW DEFINITION ON [dbo].[MakeSubmissionRequestJobInBroker] TO [Limited_Table_Write] AS [dbo]
GO
