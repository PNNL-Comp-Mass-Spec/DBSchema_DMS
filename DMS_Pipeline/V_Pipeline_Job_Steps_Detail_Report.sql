...
SELECT JS.Job_Plus_Step AS ID,
       ...
       CASE WHEN JS.State = 4 THEN CONVERT(varchar(12), CONVERT(decimal(9, 2), PS.Progress)) + '% complete' 
		     WHEN JS.State = 5 THEN 'Complete'
		     ELSE 'Not started' END AS Job_Progress,
		CASE WHEN JS.State = 4 AND JS.Step_Tool = 'XTandem' THEN 0						-- We cannot predict runtime for X!Tandem jobs since progress is not properly reported
		     WHEN JS.State = 4 AND PS.Progress > 0     THEN CONVERT(DECIMAL(9,2), DATEDIFF(second, JS.Start, ISNULL(JS.Finish, GetDate())) / 60.0 / (PS.Progress / 100.0) / 60.0)
...
GO
GRANT VIEW DEFINITION ON [dbo].[V_Pipeline_Job_Steps_Detail_Report] TO [PNL\D3M578] AS [dbo]
GO
GRANT VIEW DEFINITION ON [dbo].[V_Pipeline_Job_Steps_Detail_Report] TO [PNL\D3M580] AS [dbo]
GO
