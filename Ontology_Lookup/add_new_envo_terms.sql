...
*****************************************************/
(
    @sourceTable varchar(24) = 'T_Tmp_ENVO',
    @infoOnly tinyint = 1,
...
AS
    ...
    ---------------------------------------------------
    -- Set MatchesExisting to 1 for rows that match an existing row in T_CV_ENVO
    ---------------------------------------------------
    --
    UPDATE #Tmp_SourceData
    ...
    Begin -- <a1>
        ...
        SELECT 'Updated ' + Cast(@myRowCount as varchar(9)) + ' rows in T_CV_ENVO using ' + @sourceTable AS Message
        ...
        SELECT 'Added ' + Cast(@myRowCount as varchar(9)) + ' new rows to T_CV_ENVO using ' + @sourceTable AS Message
        ...
        FROM ( SELECT DISTINCT Identifier, Term_Name FROM T_CV_ENVO GROUP BY Identifier, Term_Name ) UniqueQTarget
             LEFT OUTER JOIN
        ...
        Begin -- <b>
            ...
            Begin -- <c>
                ...
                Begin -- <d>
                    ...
                    Else
                    Begin-- <e>
                        If Exists (Select * FROM T_CV_ENVO WHERE Identifier = @identifier AND Not Entry_ID IN (Select Entry_ID FROM #Tmp_IDsToDelete))
                        Begin
                            -- Safe to delete
                            DELETE FROM T_CV_ENVO
