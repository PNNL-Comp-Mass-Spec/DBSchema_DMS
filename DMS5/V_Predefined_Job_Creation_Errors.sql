...
CREATE VIEW [dbo].[V_Predefined_Job_Creation_Errors]
AS
SELECT SQ.Item,
...
WHERE Result_Code <> 0 AND
      SQ.State Not In ('ErrorIgnore', 'Skipped') AND
      Last_Affected >= DATEADD(DAY, -14, GETDATE())	-- Report errors within the last 14 days


GO
GRANT VIEW DEFINITION ON [dbo].[V_Predefined_Job_Creation_Errors] TO [DDL_Viewer] AS [dbo]
GO
