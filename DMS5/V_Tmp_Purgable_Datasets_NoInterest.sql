/****** Object:  View [dbo].[V_Tmp_Purgable_Datasets_NoInterest] ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[V_Tmp_Purgable_Datasets_NoInterest]
AS
SELECT DS.Dataset_ID,
       ...
       CASE
         WHEN ISNULL(DA.AS_purge_holdoff_date, GETDATE()) <= GETDATE() THEN DA.Purge_Priority
         ELSE DA.Purge_Priority + 1
       END AS Purge_Priority,
       DA.AS_state_ID AS Archive_State_ID,
       DA.Purge_Priority AS Original_Purge_Priority,
...
WHERE (InstClass.is_purgable > 0) AND
      ...
      -- ((DS.DS_created <= DATEADD(DAY, -30, GETDATE()) AND (AJ.AJ_created IS NULL OR AJ.AJ_created <= DATEADD(DAY, -30, GETDATE()))) OR
...
GROUP BY DS.Dataset_ID, SPath.SP_machine_name, SPath.SP_vol_name_server, 
         ...
         DA.Purge_Priority, DA.AS_state_ID, DA.AS_purge_holdoff_date
