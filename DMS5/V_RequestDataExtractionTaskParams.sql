...
SELECT     A.AJ_jobID AS jobID, T.AJT_resultType AS ToolResultType, T.AJT_toolName AS ToolName, D.Dataset_Num AS DatasetNum,
                      D.DS_folder_name AS DatasetFolderName, A.AJ_parmFileName AS ParamFileName, A.AJ_settingsFileName AS SettingsFileName,
                      T.AJT_parmFileStoragePath AS ParamFileStoragePath, A.AJ_comment AS Comment, A.AJ_assignedProcessorName AS AssignedProcessor,
...
GO
GRANT VIEW DEFINITION ON [dbo].[V_RequestDataExtractionTaskParams] TO [DDL_Viewer] AS [dbo]
GO
