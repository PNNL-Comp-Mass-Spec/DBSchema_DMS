...
ALTER Procedure dbo.StoreProjectUsageStatsBackfillSamples
/****************************************************
**
**	Desc: 
...
**		
**	Auth:	mem
**	Date:	12/18/2015 mem - Initial version of StoreProjectUsageStats
**			05/06/2016 mem - Updated to backfill experiments
**    
*****************************************************/
(	
	@WindowDays int = 7,
...
AS
	...
	Begin
		...
		MERGE [dbo].[T_Project_Usage_Stats] AS t
		USING (
			SELECT StartDate,
				EndDate,
				TheYear,
			...
			WHERE Not Proposal_ID Is Null) as s
		ON ( CAST(t.StartDate AS date) = CAST(s.StartDate AS date) AND
		     CAST(t.EndDate AS date) = CAST(s.EndDate AS date) AND
		     t.Proposal_ID = s.Proposal_ID AND
		     t.[TheYear] = s.[TheYear] AND
		     t.[WeekOfYear] = s.[WeekOfYear] AND
		     t.RDS_WorkPackage = s.RDS_WorkPackage
		)
		WHEN MATCHED AND t.Samples is null
		THEN UPDATE SET 
			[Samples] = s.[Samples]
		WHEN NOT MATCHED BY TARGET THEN
			INSERT([StartDate], [EndDate], [TheYear], [WeekOfYear], [Proposal_ID], [RDS_WorkPackage], [Proposal_Active], [Project_Type_ID], [Samples], [Datasets], [Jobs], [EUS_UsageType], [Proposal_Type], [Proposal_User], [Instrument_First], [Instrument_Last], [JobTool_First], [JobTool_Last])
			VALUES(s.[StartDate], s.[EndDate], s.[TheYear], s.[WeekOfYear], s.[Proposal_ID], s.[RDS_WorkPackage], s.[Proposal_Active], s.[Project_Type_ID], s.[Samples], s.[Datasets], s.[Jobs], s.[EUS_UsageType], s.[Proposal_Type], s.[Proposal_User], s.[Instrument_First], s.[Instrument_Last], s.[JobTool_First], s.[JobTool_Last])
		;
