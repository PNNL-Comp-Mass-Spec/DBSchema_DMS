...
SELECT
  ...
  TDASN.archive_state AS [Archive State],
...
FROM
  dbo.T_Dataset AS TD
  INNER JOIN dbo.T_Dataset_State_Name AS TDSN ON TD.DS_state_ID = TDSN.Dataset_state_ID
  INNER JOIN dbo.T_Instrument_Name AS TIN ON TD.DS_instrument_name_ID = TIN.Instrument_ID
  INNER JOIN dbo.T_Dataset_Type_Name DTN ON TD.DS_type_ID = DTN.DST_Type_ID
  INNER JOIN dbo.T_Experiments AS TE ON TD.Exp_ID = TE.Exp_ID
  INNER JOIN dbo.T_Users U ON TD.DS_Oper_PRN = U.U_PRN
  INNER JOIN dbo.T_Dataset_Rating_Name AS TDRN ON TD.DS_rating = TDRN.DRN_state_ID
  ...
  LEFT OUTER JOIN dbo.T_Dataset_Archive_State_Name AS TDASN ON TDASN.archive_state_id = TDA.AS_state_ID
