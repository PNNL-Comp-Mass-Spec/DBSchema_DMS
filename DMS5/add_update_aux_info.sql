/****** Object:  StoredProcedure [dbo].[add_update_aux_info] ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[add_update_aux_info]
/****************************************************
**
...
AS
    ...
    Exec @authorized = verify_sp_authorized 'add_update_aux_info', @raiseError = 1
    If @authorized = 0
    Begin;
    ...
    FROM dbo.parse_delimited_list_ordered(@itemNameList, '!', 0)
    ORDER BY EntryID


    INSERT INTO @tblAuxInfoValues (EntryID, ItemValue)
    SELECT EntryID, Value
    FROM dbo.parse_delimited_list_ordered(@itemValueList, '!', 0)
    ORDER BY EntryID
