...
AS
SELECT     DS.Dataset_ID AS ID, '' AS [Sel.], DS.Dataset_Num AS Dataset, LCC.Cart_Name AS [LC Cart], RRH.RDS_BatchID AS Batch, RRH.ID AS Request, 
                      DRN.DRN_name AS Rating, DS.DS_comment AS Comment, DSN.DSS_name AS State, InstName.IN_name AS Instrument, DS.DS_created AS Created, 
                      DS.DS_Oper_PRN AS [Oper.]
FROM         dbo.T_LC_Cart AS LCC INNER JOIN
                      dbo.T_Requested_Run AS RRH ON LCC.ID = RRH.RDS_Cart_ID RIGHT OUTER JOIN
                      ...
                      dbo.T_DatasetRatingName AS DRN ON DS.DS_rating = DRN.DRN_state_ID ON RRH.DatasetID = DS.Dataset_ID
