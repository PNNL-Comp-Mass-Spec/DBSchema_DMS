...
FROM
  T_LC_Cart AS LCC
  INNER JOIN T_Requested_Run AS RRH ON LCC.ID = RRH.RDS_Cart_ID
  RIGHT OUTER JOIN T_DatasetStateName AS DSN
  INNER JOIN T_Dataset AS DS ON DSN.Dataset_state_ID = DS.DS_state_ID
  INNER JOIN T_Instrument_Name AS InstName ON DS.DS_instrument_name_ID = InstName.Instrument_ID
  INNER JOIN T_DatasetRatingName AS DRN ON DS.DS_rating = DRN.DRN_state_ID ON RRH.DatasetID = DS.Dataset_ID
