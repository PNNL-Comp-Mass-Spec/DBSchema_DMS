...
SELECT     TOP 100 PERCENT 
	TD.Dataset_Num AS Dataset, TD.Dataset_ID AS Dataset_ID, TD.DS_folder_name AS Folder, 
	...
	AS priority, dbo.DatasetPreference(TD.Dataset_Num) AS Preference
FROM         
...
GROUP BY 
	TD.Dataset_Num, TD.Dataset_ID, TD.DS_folder_name, t_storage_path.SP_machine_name, t_storage_path.SP_vol_name_server, 
	...
	T_Dataset_Archive.AS_state_ID, dbo.DatasetPreference(TD.Dataset_Num)
HAVING      
	(NOT (ISNULL(T_Analysis_Job.AJ_StateID, 0) IN (2, 3, 9, 10, 11, 12))) AND 
	(T_Dataset_Archive.AS_state_ID = 1)
ORDER BY dbo.DatasetPreference(TD.Dataset_Num) DESC, MIN(ISNULL(T_Analysis_Job.AJ_priority, 99)), TD.Dataset_ID 
