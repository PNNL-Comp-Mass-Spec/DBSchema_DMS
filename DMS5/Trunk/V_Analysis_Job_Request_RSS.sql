...
FROM         (SELECT     T_Analysis_Job_Request.AJR_requestID, T_Analysis_Job_Request.AJR_requestName, COUNT(T_Analysis_Job.AJ_jobID) AS Total_Jobs, 
                                              ...
                                              MAX(T_Analysis_Job.AJ_finish) AS Jobs_Finished, T_Users.U_Name AS Requester, T_Users.U_PRN
                       FROM          T_Analysis_Job INNER JOIN
                                              T_Analysis_Job_Request ON T_Analysis_Job.AJ_requestID = T_Analysis_Job_Request.AJR_requestID INNER JOIN
                                              T_Users ON T_Analysis_Job_Request.AJR_requestor = T_Users.ID
                       GROUP BY T_Analysis_Job_Request.AJR_requestName, T_Analysis_Job_Request.AJR_requestID, T_Users.U_Name, 
                                              T_Analysis_Job_Request.AJR_state, T_Users.U_PRN
                       HAVING      (MAX(T_Analysis_Job.AJ_finish) > DATEADD(DAY, - 30, GETDATE())) AND (T_Analysis_Job_Request.AJR_requestID > 1)) AS T
WHERE     (Total_Jobs = Failed_Jobs + Completed_Jobs)


GO
GRANT VIEW DEFINITION ON [dbo].[V_Analysis_Job_Request_RSS] TO [PNL\D3M578] AS [dbo]
GO
GRANT VIEW DEFINITION ON [dbo].[V_Analysis_Job_Request_RSS] TO [PNL\D3M580] AS [dbo]
GO
