...
AS
SELECT     RR.ID AS Request, RR.RDS_Name AS Name, RR.RDS_Status AS Status, RR.RDS_priority AS Priority, C.Campaign_Num AS Campaign, 
                      E.Experiment_Num AS Experiment, dbo.ExpSampleLocation(RR.Exp_ID) AS [Sample Storage], RR.RDS_instrument_name AS Instrument, 
                      DTN.DST_Name AS Type, RR.RDS_Sec_Sep AS [Separation Type], U.U_Name AS Requester, RR.RDS_Oper_PRN AS PRN, 
                      RR.RDS_created AS Created, RR.RDS_Origin AS Origin, RR.RDS_instrument_setting AS [Instrument Settings], 
                      RR.RDS_special_instructions AS [Special Instructions], RR.RDS_note AS Note, RR.RDS_comment AS Comment, 
                      RR.RDS_Well_Plate_Num AS [Well Plate], RR.RDS_Well_Num AS Well, RRB.Batch AS [Batch Name], RR.RDS_BatchID AS Batch, 
                      RR.RDS_Blocking_Factor AS [Blocking Factor], RR.RDS_Block AS Block, RR.RDS_Run_Order AS [Run Order], 
