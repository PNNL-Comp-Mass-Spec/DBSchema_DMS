...
SELECT     CONVERT(varchar(32), AJ.AJ_jobID) AS JobNum, DS.Dataset_Num AS DatasetNum, DS.DS_folder_name AS DatasetFolderName, 
                      ...
                      AJ.AJ_settingsFileName AS SettingsFileName, Tool.AJT_parmFileStoragePath AS ParmFileStoragePath, 
                      AJ.AJ_organismDBName AS legacyFastaFileName, AJ.AJ_proteinCollectionList AS ProteinCollectionList, AJ.AJ_proteinOptionsList AS ProteinOptions, 
                      AJ.AJ_comment AS Comment, Inst.IN_class AS InstClass, Tool.AJT_parmFileStoragePath + 'SettingsFiles\' AS SettingsFileStoragePath, 
                      InstClass.raw_data_type AS RawDataType, Tool.AJT_searchEngineInputFileFormats AS SearchEngineInputFileFormats, 
                      Org.OG_name AS OrganismName, Tool.AJT_orgDbReqd AS OrgDbReqd, SP.SP_vol_name_client +
                          (SELECT     Client
                            FROM          T_MiscPaths
