...
AS
SELECT AJ.AJ_jobID AS Job, CASE WHEN AJ.AJ_StateID = 1 AND 
    DA.AS_state_ID IN (4, 7) 
    THEN ASN.AJS_name + ' (Dataset Purged)' WHEN AJ.AJ_StateID
     = 1 AND 
    ...
    DA.AS_state_ID > 3 THEN ASN.AJS_name + ' (Dataset ' + DASN.DASN_StateName
     + ')' ELSE ASN.AJS_name END AS Job_State, 
