...
SELECT     TD.Dataset_Num AS Dataset, TE.Experiment_Num AS Experiment, TE.EX_organism_name AS Organism, TIN.IN_name AS Instrument, 
                      ...
                      TDRN.DRN_name AS Rating, dbo.T_Requested_Run_History.ID AS Request, TD.DS_created AS Created, TD.DS_folder_name AS [Folder Name], 
                      TDSN.DSS_name AS State, dbo.V_Dataset_Folder_Paths.Dataset_Folder_Path AS [Dataset Folder Path], TDASN.DASN_StateName AS [Archive State], 
                      dbo.V_Dataset_Folder_Paths.Archive_Folder_Path AS [Archive Folder Path], TD.DS_Comp_State AS [Compressed State], 
                      TD.DS_Compress_Date AS [Compressed Date], A.Jobs, TD.Dataset_ID AS ID, TD.Acq_Time_Start AS [Acquisition Start], 
