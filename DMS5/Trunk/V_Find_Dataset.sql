...
SELECT     DS.Dataset_ID AS ID, DS.Dataset_Num AS Dataset, Exp.Experiment_Num AS Experiment, C.Campaign_Num AS Campaign, DSN.DSS_name AS State, 
                      ...
                      DFP.Archive_Folder_Path AS [Archive Folder Path], ISNULL(DS.Acq_Time_Start, RRH.RDS_Run_Start) AS [Acq Start], DATEDIFF(minute, 
                      ISNULL(DS.Acq_Time_Start, RRH.RDS_Run_Start), ISNULL(DS.Acq_Time_End, RRH.RDS_Run_Finish)) AS [Acq Length], 
                      DS.Scan_Count AS [Scan Count], LC.SC_Column_Number AS [LC Column], RRH.RDS_Blocking_Factor AS [Blocking Factor], RRH.RDS_Block AS Block, 
...
FROM         dbo.T_DatasetStateName AS DSN INNER JOIN
                      ...
                      dbo.T_LC_Column AS LC ON DS.DS_LC_column_ID = LC.ID LEFT OUTER JOIN
