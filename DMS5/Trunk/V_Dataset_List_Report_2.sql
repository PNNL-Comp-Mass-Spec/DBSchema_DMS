...
SELECT        DS.Dataset_ID AS ID, DS.Dataset_Num AS Dataset, Exp.Experiment_Num AS Experiment, C.Campaign_Num AS Campaign, DSN.DSS_name AS State, 
                         ...
                         AS [Acq. End], DATEDIFF(MINUTE, ISNULL(DS.Acq_Time_Start, RRH.RDS_Run_Start), ISNULL(DS.Acq_Time_End, RRH.RDS_Run_Finish)) AS [Acq Length], 
                         DS.Scan_Count AS [Scan Count], LC.SC_Column_Number AS [LC Column], DS.DS_sec_sep AS [Separation Type], RRH.RDS_Blocking_Factor AS [Blocking Factor], 
                         RRH.RDS_Block AS Block, RRH.RDS_Run_Order AS [Run Order], RRH.ID AS Request, RRH.RDS_EUS_Proposal_ID AS [EMSL Proposal], 
                         RRH.RDS_WorkPackage AS [Work Package], DASN.DASN_StateName AS [Archive State]
