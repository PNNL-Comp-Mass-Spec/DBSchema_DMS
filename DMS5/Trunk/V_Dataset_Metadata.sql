...
SELECT     TD.Dataset_Num AS Name, TD.Dataset_ID AS ID, TE.Experiment_Num AS Experiment, TIN.IN_name AS Instrument, 
                      TIN.IN_class AS [Instrument Description], TD.DS_sec_sep AS [Separation Type], dbo.T_LC_Column.SC_Column_Number AS [LC Column], 
                      TD.DS_wellplate_num AS [Wellplate Number], TD.DS_well_num AS [Well Number], dbo.T_DatasetTypeName.DST_name AS Type, 
                      dbo.T_Users.U_Name + ' (' + TD.DS_Oper_PRN + ')' AS Operator, TD.DS_comment AS Comment, TDRN.DRN_name AS Rating, 
                      dbo.T_Requested_Run_History.ID AS Request, TDSN.DSS_name AS State, TDASN.DASN_StateName AS [Archive State], TD.DS_created AS Created, 
                      TD.DS_folder_name AS [Folder Name], TD.DS_Comp_State AS [Compressed State], TD.DS_Compress_Date AS [Compressed Date], 
                      TD.Acq_Time_Start AS [Acquisition Start], TD.Acq_Time_End AS [Acquisition End], TD.Scan_Count AS [Scan Count], CONVERT(int, 
                      TD.File_Size_Bytes / 1024.0 / 1024.0) AS [File Size MB]
FROM         dbo.T_Dataset AS TD INNER JOIN
                      ...
                      dbo.T_LC_Column ON TD.DS_LC_column_ID = dbo.T_LC_Column.ID LEFT OUTER JOIN
