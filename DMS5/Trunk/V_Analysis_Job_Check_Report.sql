...
AS
SELECT     CONVERT(varchar(32), dbo.T_Analysis_Job.AJ_jobID) AS Job, dbo.T_Analysis_State_Name.AJS_name AS State, 
                      dbo.T_Analysis_Job.AJ_start AS Started, dbo.T_Analysis_Job.AJ_finish AS Finished, ISNULL(dbo.T_Analysis_Job.AJ_assignedProcessorName, 
                      '(none)') AS CPU, dbo.T_Analysis_Tool.AJT_toolName AS [Tool Name], dbo.T_Dataset.Dataset_Num AS Dataset, 
                      dbo.T_Analysis_Job.AJ_comment AS Comment, dbo.T_Analysis_Job.AJ_priority AS [Pri.], dbo.t_storage_path.SP_machine_name AS Storage, 
                      dbo.t_storage_path.SP_path AS Path, dbo.T_Analysis_Job.AJ_parmFileName AS [Parm File], 
                      dbo.T_Analysis_Job.AJ_settingsFileName AS [Settings File], dbo.T_Analysis_Job.AJ_organismDBName AS [Organism DB], 
                      ...
                      dbo.T_Organisms.OG_name AS Organism, ISNULL(DATEDIFF(hour, dbo.T_Analysis_Job.AJ_start, GETDATE()), 0) AS Elapsed
