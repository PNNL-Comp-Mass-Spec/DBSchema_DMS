...
SELECT
  ...
  CASE WHEN NOT TPR.Uploaded IS NULL
       THEN TIS.SP_vol_name_client + TIS.SP_path
            + dbo.GetDMSFileStoragePath('', TPR.ID, 'prep_lc')
       ELSE ''
  END AS Folder,
  TPR.Uploaded
FROM
  T_Prep_LC_Run AS TPR
  LEFT OUTER JOIN T_Prep_Instrument_Storage AS TIS ON TPR.Storage_Path = TIS.SP_path_ID
GO
