...
GO
CREATE VIEW V_Notification_Analysis_Job_Request_By_Research_Team AS
...
FROM
  ...
  INNER JOIN T_Analysis_Job_Request ON TNE.Target_ID = T_Analysis_Job_Request.AJR_requestID
  INNER JOIN T_Analysis_Job ON T_Analysis_Job_Request.AJR_requestID = T_Analysis_Job.AJ_requestID
 INNER JOIN 
 (
 	 SELECT
 ...
 ) T ON T.Dataset_ID = T_Analysis_Job.AJ_datasetID
WHERE
   TET.Target_Entity_Type = 2
   AND (TET.Visible = 'Y')
GO
