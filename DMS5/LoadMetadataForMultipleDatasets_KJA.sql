/****** Object:  StoredProcedure [dbo].[LoadMetadataForMultipleDatasets_KJA] ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE LoadMetadataForMultipleDatasets_KJA
/****************************************************
**
...
As
	...
	--
	INSERT INTO #metaD(mDst, mAType, mTag, mVal)
	SELECT Name , 'Dataset', 'Campaign', MD.[Campaign]
	FROM V_Dataset_Metadata MD
	WHERE (Name IN (SELECT mDst FROM #dst))
	--
	INSERT INTO #metaD(mDst, mAType, mTag, mVal)
	SELECT Name , 'Dataset', 'Campaign Description', MD.[Campaign Description]
	FROM V_Dataset_Metadata MD
	WHERE (Name IN (SELECT mDst FROM #dst))
	--	
	INSERT INTO #metaD(mDst, mAType, mTag, mVal)
	SELECT Name , 'Dataset', 'Cell Culture List', MD.[Cell Culture List]
	FROM V_Dataset_Metadata MD
	WHERE (Name IN (SELECT mDst FROM #dst))
	--
	INSERT INTO #metaD(mDst, mAType, mTag, mVal)
...
Done:
	return @myError
GO
GRANT VIEW DEFINITION ON [dbo].[LoadMetadataForMultipleDatasets_KJA] TO [DDL_Viewer] AS [dbo]
GO
