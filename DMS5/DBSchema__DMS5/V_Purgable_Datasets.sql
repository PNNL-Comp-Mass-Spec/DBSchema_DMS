...
GO
CREATE VIEW V_Purgable_Datasets
AS
...
WHERE (dbo.T_Instrument_Class.is_purgable > 0) AND (dbo.T_Dataset_Archive.AS_state_ID = 3) AND (TD.DS_rating <> - 1) AND 
               (ISNULL(dbo.T_Dataset_Archive.AS_purge_holdoff_date, GETDATE()) <= GETDATE()) AND (NOT EXISTS
                   (SELECT *
                    FROM   T_Analysis_Job
                    WHERE AJ_StateID IN (1, 2, 3, 8, 9, 10, 11, 12) AND AJ_datasetID = TD.Dataset_ID))
