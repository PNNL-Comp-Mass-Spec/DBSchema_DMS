/****** Object:  StoredProcedure [dbo].[consume_scheduled_run] ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[consume_scheduled_run]
/****************************************************
**
**  Desc:
...
**          12/05/2011 mem - Updated call to add_requested_run_to_existing_dataset to include @DatasetName
...
**          12/14/2011 mem - Added parameter @callingUser, which is passed to add_requested_run_to_existing_dataset and alter_event_log_entry_user
...
AS
    ...
    Set @transName = 'consume_scheduled_run_' + convert(varchar(12), @datasetID) + '_' + convert(varchar(12), @requestID)

    If @logDebugMessages = 1
    Begin
        Set @LogMessage = 'Start transaction ' + @transName
        exec post_log_entry 'Debug', @LogMessage, 'consume_scheduled_run'
    End
    ...
    Begin -- <a>
        ...
        exec add_requested_run_to_existing_dataset @datasetID=@ExistingDatasetID, @datasetName='', @templateRequestID=@requestID, @mode='add', @message=@message output, @callingUser=@callingUser
        ...
        Begin -- <b1>
            ...
            exec post_log_entry 'Warning', @LogMessage, 'consume_scheduled_run'
        ...
        Begin -- <b2>
            ...
            Set @LogMessage = 'Tried to add a new automatic requested run for dataset "' + @ExistingDatasetName + '" since re-using request ' + Convert(varchar(12), @requestID) + '; however, add_requested_run_to_existing_dataset was unable to auto-create a new Requested Run'
            exec post_log_entry 'Error', @LogMessage, 'consume_scheduled_run'
    ...
    If @logDebugMessages = 1
    Begin
        Set @LogMessage = 'Call update_cached_requested_run_eus_users for ' + Cast(@requestID as varchar(12))
        exec post_log_entry 'Debug', @LogMessage, 'consume_scheduled_run'
    End
...
GO
GRANT VIEW DEFINITION ON [dbo].[consume_scheduled_run] TO [DDL_Viewer] AS [dbo]
GO
GRANT EXECUTE ON [dbo].[consume_scheduled_run] TO [DMS_SP_User] AS [dbo]
GO
GRANT EXECUTE ON [dbo].[consume_scheduled_run] TO [Limited_Table_Write] AS [dbo]
GO
GRANT VIEW DEFINITION ON [dbo].[consume_scheduled_run] TO [Limited_Table_Write] AS [dbo]
GO
