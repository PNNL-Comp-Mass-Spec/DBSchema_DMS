...
SELECT  dbo.T_Experiment_Groups.Group_ID AS ID ,
        ...
        CASE WHEN T_Experiment_Groups.Researcher IS NULL THEN ''
             ELSE T_Users.U_Name + ' (' + T_Users.U_PRN + ')'
        END AS Researcher
FROM    dbo.T_Experiment_Groups
        LEFT OUTER JOIN dbo.T_Experiment_Group_Members ON dbo.T_Experiment_Groups.Group_ID = dbo.T_Experiment_Group_Members.Group_ID
        INNER JOIN dbo.T_Experiments ON dbo.T_Experiment_Groups.Parent_Exp_ID = dbo.T_Experiments.Exp_ID
        LEFT OUTER JOIN ( SELECT    Entity_ID AS [Entity ID] ,
                          ...
                          AND Active > 0
                          GROUP BY  Entity_ID
                        ) AS TA ON dbo.T_Experiment_Groups.Group_ID = TA.[Entity ID]
        LEFT OUTER JOIN dbo.T_Users ON dbo.T_Experiment_Groups.Researcher = dbo.T_Users.U_PRN
GROUP BY dbo.T_Experiment_Groups.Group_ID ,
        dbo.T_Experiment_Groups.EG_Group_Type ,
        dbo.T_Experiment_Groups.EG_Description ,
        dbo.T_Experiment_Groups.EG_Created ,
